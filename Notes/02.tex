...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item[($\subseteq$)] We want to show that $\SE \subseteq \kM(\SE')$.  
...
\end{theorem}
\begin{proof}
  Let $\SP = \{E_1 \times E_2 : E_i \in \SE_i\}$, $\kM(\SP) \subseteq \SM_1 \oplus \SM_2$.
  ...
  then $\SG_1$ is a $\sigma$-algebra consistion of subset of $X_1$ which contains $\SE_1$, $\SE_1\subseteq \SG_1$.
...
$\bigcup E_j = E_1 \cup (E_2 \setminus E_1) \cup (E_3 \setminus (E_1 \cup E_2)) \cup \dotsm$
\end{proof}