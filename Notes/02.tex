...
\begin{proof}
  $f_n \le f_{n+1} \le f$ then $\int f_n \le \int f \implies \lim_{n\to\infty} \int f_n \le \int f$.

  We need to show $\int f \le \lim\limits_{n\to\infty} \int f_n$.
  So, it suffices to show that for any $0 \le s_\text{simple} \le f$, that 
  $\int s \le \lim\limits_{n\to\infty} \int f_n$.

  It suffices to show that for any $\eps > 0$, $(1-\eps)\int s\ \mathrm{d}\mu \le \lim \int f_n\ \mathrm{d}\mu$ (if we show this, we are done).
  Define $E_n = \{x : (1 - \eps)s(x) \le f_n(x)\}$, any $x$ will be in one of the $E_n$.
  $(1-\eps) s(x)\ \mathrm{d}\mu< \lim f_n(x) = x\ \mathrm{d}\mu$. This mean $(1-\eps)s(x) < f_n(x)$ for sufficiently large $n$ ($n \ge N(x)$).
  We will show that $\bigcup E_n = X$, consider the easure defined by $\nu(E) = \int_E s\ \mathrm{d}\mu$ (we checked that it is a measure).
  We have $E_n \subseteq E_{n+1} \to X$. Continuity, $\lim\limits_{n\to\infty}\nu(E_n) = \nu(\bigcup E_n)= \nu(X) = \int s\ \mathrm{d}\mu$.
  $$\nu(E_n) = \int s\ \mathrm{d}\mu \le \int \frac{f_n(x)}{1-\eps}\ \mathrm{d}\mu = \frac1{1-\eps}\int f_n(x)\ \mathrm{d}\mu$$
  We have 
  $$\int s\ \mathrm{d}\mu = \nu(x) = \lim_{n\to\infty}\nu(E_n) = \lim_{n\to\infty} \frac1{1-\eps} \int f_n\ \mathrm{d}\mu$$
\end{proof}
