...
\begin{proof}
  ...
  WLOG $b \le a,$
  $f(x) = 1+x^p - (1-x)^p$, $f'(x) \ge 0 \implies (1+x)^p \le 1 + x^p$ for $0 < p \le 1$.  
  set $x = b/a$ Then
  \[(a+b)^p \le a^p + b^p\] 
  then setting $a = |f(x)|, b = |g(x)|$ and we are done.
\end{proof}
...
\begin{lemma}[Shebyshev's inequality]
  ...
  $\mu_f(\alpha) = \mu(\{x : |f(x)| \ge \alpha\}), E_\alpha = \{x : |f(x)| \ge \alpha\}$
  The Shebyshev's inequality is 
  \[\mu_f(\alpha) \le \frac{\|f\|_p^p}{\alpha^p}\]
\end{lemma}
\begin{proof}
  for $x \in E_\alpha$, 
  \[1 \le \frac{|f(x)|^p}{\alpha^p}\]
  \begin{align*}
